...
float Bplus[3][1]){
    ...
    adsq[3][1], mdsq[3][1], aisq[3][1], misq[3][1], gyrosDeltaT[3][1]; 
    ...
    // transpose rminus matrix: should still be identity-ish
    ...
    VectorScalarMultiply(timeChange, gyroFeedback, gyrosDeltaT, 3);
    
    // caluclate Rplus and bplus
    //rexp(gyroFeedback, Rexpo); commented out 4/23/19 1:38
    // issue: forgot to integrate the gyro by multiplying by the time step
    rexp(gyrosDeltaT, Rexpo);
...
void ClosedLoop(float bias[3][1], float Rminus[3][3], float gyroInput[3][1], float magData[3][1], 
...
float kp_m, float ki_a, float ki_m, float BetterR[3][3], float biasEst[3][1]){
    ...
    for(i = 0; i < steps; i++){
        ...
        IntegrateClosedLoop(RCopy, bias, realGyroCopy, magData, accelData, 
        magIn, accelIn, 0.05, kp_a, kp_m, ki_a, ki_m, BetterR, biasEst);

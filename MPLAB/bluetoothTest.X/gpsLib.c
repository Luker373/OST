...
float longitude=0, latitude=0, alt=0, speed;
int time[3];
...
void createGNTST(char *str){
        ...
        sprintf(out, "$GNTST,%.2f,%d,%d,%.2f,%.2f,", getSpeed(), -1,-1,-1.0,-1.0);
        if(getHour() < 10)
          strcat(out, "0");
...
void createGNOST(char *str){
        ...
        sprintf(out, "$GNOST,%d,%.2f,%.2f,%d,%d,%.2f,%.2f,", -1, getLat()/100, getLong()/100, -1,-1,-1.0,-1.0);
        if(getHour() < 10)
          strcat(out, "0");
        itoa(buffer, getHour(), 10);
        strcat(out, buffer);
...
// $GPRMC,time,Active?,lat,N,long,E,speed(knots),track(degrees),date,mag variation,W*6A

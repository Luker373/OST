%% CALCULATE TRIAD VECTORS FROM RAW %%
...
for c = 1:length(appWind)
    ...
    appWindSpeed(c) = sqrt(appWind(c,1)^2 + appWind(c,2)^2); %uneccessary (same as anem windSpeed)
end
...
%% VARIABLES DETERMINE SAMPLING RESULTS%%
num_wS = 25;
base_wS = 1;
range_wS = 0.5;
%%%%
...
% calculate/pull points of sails and corresponding boat speeds from raw dat
for k = 1:num_wS    
...
% examine for best butterfly plot (change sample vars if nec.)
...
%best sample? trim of half the data
sample = 7;
%%
pos2 = pos(:,sample);
...
% average the boat speed around dTheta thresh
dTheta = 10;
k = 1;
...
% plot chosen sample polar speed plot
    phi = 1:180;
    figure(9)    

...
void createGNOST(char *str){
        ...
        sprintf(out, "$GNOST,%.2f,%d,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,",-1, getLat()/100, getLong()/100, getWindSpeed(),getWindDegree(), getSpeed(),track,getBoomAngle());
...
void setBoomAngle(float f){
    boomAngle = i;
}

float getBoomAngle(void){
    return boomAngle;
}
